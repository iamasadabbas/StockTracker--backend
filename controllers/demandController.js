...
exports.demandProduct = catchAsyncError(async (req, res, next) => {
  const { products, subject, applicationId, date, locationId, user_id, signatureRecord_id } = req.body;
...
exports.getDemandbyId = catchAsyncError(async (req, res, next) => {
  try {
    const demandedProduct = await Demand.findOne({
      _id: req.params._id
    }).populate('products._id').populate('signatureRecord_id')
